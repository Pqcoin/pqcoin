...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    //cmx add coinbasetx for funding
    coinbaseTx.vout[0].nValue = nFees + GetPqcoinBlockSubsidy(nHeight, consensus, pindexPrev->GetBlockHash(
));
    CBitcoinAddress fundingAddress("DURqHAqv77AoiqXxvLZmV4BtWbgSDpmn4G");
    ...
    //wh_init_ltc_nBits
    if (ltc_nBits.empty()) ltc_nBits.push_back(UintToArith256(consensus.powLimit).GetCompact());
    if (btc_nBits.empty()) btc_nBits.push_back(UintToArith256(consensus.powLimit).GetCompact());
