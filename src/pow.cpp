...
bool AllowMinDifficultyForBlock(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Pqcoin: Magic number at which reset protocol switches
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Pqcoin: Special rules for minimum difficulty blocks with Digishield
...
//        std::cout<<"now it is the first mine block:"<<std::endl;
    ...
    assert(pindexFirst);
    // ***jsrchange27
    return CalculatePqcoinNextWorkRequired_new(pindexLast, pindexFirst->GetBlockTime(), params);
    // return CalculatePqcoinNextWorkRequired(pindexLast, pindexFirst->GetBlockTime(), params);
}
