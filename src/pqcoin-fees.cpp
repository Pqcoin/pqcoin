...
CFeeRate GetPqcoinFeeRate(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
        return CFeeRate(COIN / 100 * 521); // 5.21 PQ, but very carefully avoiding floating point maths
    case MANY_GENEROUS:
...
const std::string GetPqcoinPriorityLabel(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
CAmount GetPqcoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    {
        LOCK(mempool.cs);
    ...
    nMinFee += GetPqcoinDustFee(tx.vout, nDustLimit);
...
CAmount GetPqcoinDustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
