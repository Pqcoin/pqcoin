...
extern double change_to_nbits(unsigned int i);
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated pqcoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Pqcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Pqcoin is downloading blocks...");
...
static UniValue AuxMiningCreateBlock(const CScript& scriptPubKey)
{
     ...
     * a single pqcoind instance, for example when a pool runs multiple sub-
...
static UniValue newAuxMiningSubmitBlock(const std::string& hashHex, const std::string& auxpowHex)
{
    ...
    if (block.auxpow->parentBlock.nVersion == 0x10000000) {
        ret = 1;
    } else if (block.auxpow->parentBlock.nVersion == 0x20000000 ) ret = 2;  
...
//cmx_newAuxMiningCreateBlock
static UniValue newAuxMiningCreateBlock(const CScript& scriptPubKey, int id)
{
    // AuxMiningCheck();
    // LOCK(cs_auxblockCache);
    static unsigned nTransactionsUpdatedLast;
    ...
    std::cout<<"nbit from block:"<<nbits<<endl;
...
//wh_newAuxMiningCreateBlock_define
static void newAuxMiningCreateBlock(const CScript& scriptPubKey, uint256 &hash, uint32_t &nbits, int id)
{
    // AuxMiningCheck();
    // LOCK(cs_auxblockCache);
    static unsigned nTransactionsUpdatedLast;
...
static bool AuxMiningSubmitBlock(const std::string& hashHex, const std::string& auxpowHex)
{
 ...
 static int cmx_submit_Auxblock(const std::string hashHex , const std::string auxpowHex){
    ...
    uint256 hash;
    hash.SetHex(hashHex) ;
    std::cout<<hash.ToString()<<endl;
    string str_auxpow = auxpowHex;
    int size = str_auxpow.size();
    const char *auxpow = str_auxpow.c_str();
    const char *ptr = auxpow;
    const std::map<uint256, CBlock*>::iterator mit = mapNewBlock.find(hash);
    ...
    //nversion
    memcpy(&block.auxpow->parentBlock.nVersion, ptr, 4);
    ptr += 4;
    int ret = 0;
    if (block.auxpow->parentBlock.nVersion == 0x10000000) {
        ret = 1;
    } else if (block.auxpow->parentBlock.nVersion == 0x20000000 ) ret = 2;   
    //parent hashPrevBlock 
    memcpy(block.auxpow->parentBlock.hashPrevBlock.begin(), ptr, 32);
    ptr += 32;
    //parent hashMerkleRoot
    memcpy(block.auxpow->parentBlock.hashMerkleRoot.begin(), ptr, 32);
    ptr += 32;
    //nTime
    memcpy(&block.auxpow->parentBlock.nTime, ptr, 4);
    ptr += 4;
    //nBits
    memcpy(&block.auxpow->parentBlock.nBits, ptr, 4);
    ptr += 4;
    //nNonce
    memcpy(&block.auxpow->parentBlock.nNonce, ptr, 4);
    ptr += 4;
    // parent hash 
    memcpy(block.auxpow->hashBlock.begin(), ptr, 32);
    ptr += 32;
    //CoinBase_Branch
    uint32_t coinBaseBranchSize = 0;
    memcpy(&coinBaseBranchSize, ptr, 4);
    ptr += 4;
    if (coinBaseBranchSize > 0) {
        block.auxpow->vMerkleBranch.resize(coinBaseBranchSize / sizeof (uint256));
        memcpy(&block.auxpow->vMerkleBranch[0], ptr, coinBaseBranchSize);
    }
    ptr += coinBaseBranchSize;
    //write coinBaseHash in auxpow->hashBlock 
    memcpy(block.auxpow->hashBlock.begin(), ptr, 32);
    ptr += 32;
    ...
    if (fAccepted) {
        std::cout << "\n===============================================================================" << std::endl;
        std::cout << "childHash:" << hash.GetHex() << std::endl;
        std::cout << "from: ";
        if (shared_block->auxpow->parentBlock.nVersion == 0x10000000) {
            std::cout << "litecoin";
        } else std::cout << "bitcoin";
        std::cout << std::endl;
        std::cout << "prtPreHash: " <<shared_block->auxpow->parentBlock.hashPrevBlock.GetHex() << std::endl;
        std::cout << "ptrMRHash: " << shared_block->auxpow->parentBlock.hashMerkleRoot.GetHex() << std::endl;        
        std::cout << "nTime: " << shared_block->auxpow->parentBlock.nTime << std::endl;
        if (shared_block->auxpow->parentBlock.nVersion == 0x10000000) {
            std::cout << "auxbits: " << change_to_nbits(hash_to_nBits[shared_block->GetHash().GetHex()].first)  << std::endl;
        } else std::cout << "auxbits: " << change_to_nbits(hash_to_nBits[shared_block->GetHash().GetHex()].second)  << std::endl;
        std::cout << "prtbits: " << change_to_nbits(shared_block->auxpow->parentBlock.nBits) << std::endl; 
        std::cout << "nNonce: " << shared_block->auxpow->parentBlock.nNonce << std::endl;        
        std::cout << "prtbits: " << shared_block->auxpow->parentBlock.GetHash().GetHex() << std::endl;       
        std::cout << "branch size: " << coinBaseBranchSize << std::endl;       
        std::cout << "cBaseHash: " << shared_block->auxpow->hashBlock.GetHex() << std::endl;        
        std::cout << "cBase size: " << coinBaseSize << std::endl;
        std::cout << "sucess count: " << ++count;
        if (ret == 1) ++count_ltc;
        else ++count_btc;
        std::cout << " from btc: " << count_btc << " from ltc: " << count_ltc << std::endl; 
        std::cout << "===============================================================================\n" << std::endl;
...
//cmx_submit_Auxblock
UniValue SubmitAuxBlock(JSONRPCRequest& request){
    
   
    int re = cmx_submit_Auxblock(request.params[0].get_str(),request.params[1].get_str());
    
    return  UniValue(re);
}


//wh_SubmitAuxBlock_define
int SubmitAuxBlock(const char buf[])
{
    const char *ptr = buf;
    uint256 hash;
    memcpy(hash.begin(), ptr, 32);
    ptr += 32;
    const std::map<uint256, CBlock*>::iterator mit = mapNewBlock.find(hash);
    ...
    //nversion
    memcpy(&block.auxpow->parentBlock.nVersion, ptr, 4);
    std::cout<<"nversion"<<block.auxpow->parentBlock.nVersion<<endl;
    ptr += 4;
    int ret = 0;
    if (block.auxpow->parentBlock.nVersion == 0x10000000) {
        ret = 1;
...
//cmx_createauxblock_define with univalue
...
UniValue createauxblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "createauxblock <address><hash><nbits><id>\n"
            "\ncreate a new block and return information required to merge-mine it.\n"
            "\nArguments:\n"
            "1.address      (string, required) specify coinbase transaction payout address\n"
            "2.id           (numeric, required) specify the id of the block to be created\n"
            "\nResult:\n"
            "{\n"
            "  \"hash\"               (string) hash of the created block\n"
            "  \"chainid\"            (numeric) chain ID for this block\n"
    ...
    // list<UniValue::VType> l;
    // l.push_back(UniValue::VSTR);
    // l.push_back(UniValue::VSTR);
    // l.push_back(UniValue::VSTR);
    // l.push_back(UniValue::VSTR);
    
    // RPCTypeCheck(request.params, l);
    CBitcoinAddress coinbaseAddress(request.params[0].get_str());
    int id;
    if(request.params[1].isNum()){
        id = request.params[1].get_int();
    }
    else{
        string strid = request.params[1].get_str();
        id =stringToInt(strid);
    }
    ...
    return newAuxMiningCreateBlock(scriptPubKey,id);
}

//wh_createauxblock_define
bool createauxblock(const string& address, uint256 &hash, uint32_t &nbits, int id)
{
    CBitcoinAddress coinbaseAddress(address);
    if (!coinbaseAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_PARAMETER,"Invalid coinbase payout address");
    const CScript scriptPubKey = GetScriptForDestination(coinbaseAddress.Get());
    newAuxMiningCreateBlock(scriptPubKey, hash, nbits, id);
    return true;
}
...
UniValue submitauxblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "ret          (numeric) whether the submitted block was correct and its type\n"
            "\nExamples:\n"
            + HelpExampleCli("submitauxblock", "\"hash\" \"serialised auxpow\"")
            + HelpExampleRpc("submitauxblock", "\"hash\" \"serialised auxpow\"")
            );

    return newAuxMiningSubmitBlock(request.params[0].get_str(),
                                request.params[1].get_str());
}
/*
Create or submit a merge-mined block.Without arguments, create a new block and return information required to merge-mine it.  
