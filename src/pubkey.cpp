 ...
 *  at the end, and overly lonECC_InitSanityCheckg length descriptors. This is safe to use in
...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
    if (!IsValid())
        return false;
    int r = crypto_sign_verify(vchSig.data(),vchSig.size(),hash.begin(),32,begin()+1);
    if( r == 0){
        return true;
    }else {
        printf("\n--- verify is failed.\n");
...
bool CPubKey::RecoverCompact(const uint256 &hash, const std::vector<unsigned char>& vchSig) {
    ...
    pch[0]=7;
    int r = crypto_sign_verify(vchSig.data(),mlen,hash.begin(),32,pch+1);
    if( r == 0){
        return true;
    }else {
