Pqcoin Core version 1.14.5 is now available from:

  <https://github.com/pqcoin/pqcoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All Pqcoin Core users, miners, services, relay
...
To receive security and update notifications, please watch reddit or Twitter:
  ...
  * @Pqcoin on Twitter for high priority announcements
  * @pqcoin\_devs on Twitter for updates on development work
...
most Pqcoin Core users.

### Remote Code Execution in Pqcoin QT (CVE-2021-3401)
...
**Pqcoin QT users are urged to please update their installations to this
...
leakage of sensitive information when Pqcoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**Pqcoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the Pqcoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 PQ/kb, and
* the recommended dust limit is 1 PQ, and
* the recommended RBF increment is 0.001 PQ.

### Wallet/UI Changes

* The user interface for selecting fees when transacting PQ has been updated
  to give an idea of how much is being spent, rather than a block target. As
  Pqcoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for Pqcoin.
...
* The default minimum transaction fee is now 0.01 PQ per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 PQ, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 PQ.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 PQ and sets the value under
...
The Berkley DB version used by Pqcoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within Pqcoin Core.
