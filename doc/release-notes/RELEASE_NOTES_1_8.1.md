...
Pqcoin Core 1.8.1 is primarily a bugfix release, bringing Pqcoin Core in
line with Bitcoin 0.9.3. Pqcoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Pqcoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Pqcoin Core clients
* Use HTTPS with pqchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/pqcoin/pqcoin/issues/134).
...
This only affects values significantly below 1 Pq, however merchants may wish to round to
the nearest Pq on all transactions for now.
