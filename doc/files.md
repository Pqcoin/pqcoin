...
The data directory is the default location where the Pqcoin Core files are stored.
...
Windows  | `%APPDATA%\Pqcoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `pqcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `pqcoin.conf`       | User-defined configuration settings for `pqcoind` or `pqcoin-qt`; can be specified by `-conf` option
`./`               | `pqcoind.pid`        | Stores the process ID (PID) of `pqcoind` or `pqcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `pqcoind` or `pqcoin-qt`
...
`pqcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Pqcoin Core:
