...
You must install required dependencies to build a basic Pqcoin daemon, optional dependencies may vary according to your requirements.
...
Pqcoin Core's released binaries are built using the "depends system", which
...
According to installed dependencies, the following steps will compile `pqcoind`, `pqcoin-cli` and `pqcoin-qt`.
...
#### Enable Qt GUI
Create `pqcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Pqcoin Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Pqcoin Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Pqcoin may be compiled in
...
#### Security
To help make your Pqcoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Pqcoin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Pqcoin Core.
