...
Below are some notes on how to build Pqcoin Core for Windows.
...
Note that for WSL v1 the Pqcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/pqcoin, AND not under, for example, /mnt/d/pqcoin. 
...
way. This will install to `c:\workspace\pqcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/pqcoin
